...
# --- Helper: Run C++ Demo A* Search ---
def run_cpp_demo_astar(cpp_module, cpp_network, start_node, end_node):
    """Runs the C++ A* implementation and returns the path and execution time."""
    print("\nRunning C++ Demo A* implementation...")
    start_time = time.time()
    cpp_path = None
    try:
...
def run_cpp_sequential_astar(cpp_module, cpp_network, start_node, end_node):
    """Runs the C++ A* implementation and returns the path and execution time."""
    print(f"\nRunning C++ Sequential A* implementation...")
    start_time = time.time()
    cpp_path = None
    try:
        # Access the function within the 'demo' submodule
        cpp_path = cpp_module.demo.astar_search_sequential(cpp_network, start_node, end_node)
        cpp_time = time.time() - start_time
        if not cpp_path:  # C++ returns empty list [] if no path
            print(f"C++ A*: No path found in {cpp_time:.4f} seconds.")
        else:
...
# --- Helper: Run C++ Sequential A* Search ---
def run_cpp_parallel_astar(cpp_module, cpp_network, start_node, end_node, number_of_threads):
    """Runs the C++ A* implementation and returns the path and execution time."""
    print(f"\nRunning C++ Parallel A* implementation with {number_of_threads} threads...")
    start_time = time.time()
    cpp_path = None
    try:
        # Access the function within the 'demo' submodule
        cpp_path = cpp_module.demo.astar_search_parallel(cpp_network, start_node, end_node, number_of_threads)
        cpp_time = time.time() - start_time
        if not cpp_path:  # C++ returns empty list [] if no path
            print(f"C++ A*: No path found in {cpp_time:.4f} seconds.")
        else:
...
def main():
    ...
    # Sequential version
    seq_cpp_path, seq_cpp_time = run_cpp_sequential_astar(
        assignment2_cpp, cpp_road_network, start_node, end_node
    )
    
    # Parallel version with 1 thread
    par_1thr_cpp_path, par_1thr_cpp_time = run_cpp_parallel_astar(
        assignment2_cpp, cpp_road_network, start_node, end_node, 1
    )
    
    # Parallel version with 2 thread
    par_2thr_cpp_path, par_2thr_cpp_time = run_cpp_parallel_astar(
        assignment2_cpp, cpp_road_network, start_node, end_node, 2
    )
    
    # Parallel version with 4 thread
    par_4thr_cpp_path, par_4thr_cpp_time = run_cpp_parallel_astar(
        assignment2_cpp, cpp_road_network, start_node, end_node, 4
    )

    # Compare results
    compare_results(seq_cpp_path, seq_cpp_time, nx_path, nx_time, G_nx, WEIGHT_ATTRIBUTE)
    compare_results(par_1thr_cpp_path, par_1thr_cpp_time, nx_path, nx_time, G_nx, WEIGHT_ATTRIBUTE)
    compare_results(par_2thr_cpp_path, par_2thr_cpp_time, nx_path, nx_time, G_nx, WEIGHT_ATTRIBUTE)
    compare_results(par_4thr_cpp_path, par_4thr_cpp_time, nx_path, nx_time, G_nx, WEIGHT_ATTRIBUTE)
