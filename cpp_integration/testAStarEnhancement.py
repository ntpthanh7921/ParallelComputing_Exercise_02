...
def run_cpp_sequential_astar(cpp_module, cpp_network, start_node, end_node):
    ...
    try:
        ...
        cpp_path = cpp_module.demo.AStarEnhancement_search(cpp_network, start_node, end_node)
...
def is_in_high_cost_zone(lat, lon):
    """Returns True if the coordinates fall within the dynamic high-cost zone."""
    return (
        HIGH_COST_ZONE["min_lat"] <= lat <= HIGH_COST_ZONE["max_lat"]
        and HIGH_COST_ZONE["min_lon"] <= lon <= HIGH_COST_ZONE["max_lon"]
    )
...
def run_cpp_parallel_astar(cpp_module, cpp_network, start_node, end_node, number_of_threads, parallel_type):
    ...
    try:
        # Access the function within the 'demo' submodule        
        parallel_dispatch = {
            "TPool___CppLib"  : cpp_module.demo.AStarEnhancementParallel_search_TPool_CppLib,
            "TVector_CppLib": cpp_module.demo.AStarEnhancementParallel_search_TVector_CppLib,
            "TPool___PqFine"  : cpp_module.demo.AStarEnhancementParallel_search_TPool_PqFine,
            "TVector_PqFine": cpp_module.demo.AStarEnhancementParallel_search_TVector_PqFine,
        }
...
def nx_heuristic(u, v, G_nx):
    """Dynamic heuristic function with regional penalty."""
    try:
        u_coords = G_nx.nodes[u]
        v_coords = G_nx.nodes[v]
        base_distance = ox.distance.great_circle(
            u_coords["y"], u_coords["x"], v_coords["y"], v_coords["x"]
        )
        
        # Dynamically modify h(v) if u is in a high-cost zone
        if is_in_high_cost_zone(u_coords["y"], u_coords["x"]):
            return base_distance + DYNAMIC_PENALTY
        else:
...
def main():
    ...
    print("--- A* algorithm when estimate of path weight is dynamic ---")
